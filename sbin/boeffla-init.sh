...
# which is used for many purposes (set permissions and owners correctly)
	if [ ! -d "$BOEFFLA_DATA_PATH" ] ; then
		...
		/system/xbin/busybox chmod 775 $BOEFFLA_DATA_PATH
		/system/xbin/busybox chown 1023:1023 $BOEFFLA_DATA_PATH
	fi
...
# If rom comes without mount command in /system/bin folder, create busybox symlinks for mount/umount
	if [ ! -f /system/bin/mount ]; then
		...
		/system/xbin/busybox ln /system/xbin/busybox /sbin/mount
		/system/xbin/busybox ln /system/xbin/busybox /sbin/umount
